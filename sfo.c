...
uint16_t bswap_16(uint16_t val) {
  val = (val & 0xFF00 ) | (val & 0xFF );
  return (val << 8) | (val >> 8);
}

uint16_t bswap_16_alt(uint16_t val) {
...
// Global variables
const char program_version[] = "1.03 (March 11, 2022)";
char *program_name;
char *query_string;
FILE *file;
int option_debug;
...
void print_key_table(void) {
  ...
  if (key_table.size) {
    ...
    for (int i = 0; i < (key_table.size-1); i++) {
      ...
      case 516:
        if (option_verbose) {
          printf("[%d] %s=\"%s\" (%d/%d bytes UTF-8 string)\n", i,
        	...
        	printf("%s=%s\n", &key_table.content[entries[i].key_offset],
            &data_table.content[entries[i].data_offset]);
            //fputs ("x", pFile); //only prints a string....
        }
        break;
      case 1024:
        if (option_verbose) {
...
void print_version(void) {
  printf("SFO (Mod by MrDude) v%s\n", program_version);
  printf("https://github.com/hippie68/sfo\n");
}

void print_myfile(void) {
  printf("Option not implemented yet!\n");
  //need to tell what file to open etc...mod this later
}
...
void create_param_sfo(char *file_name) {
  header.magic = 1179865088; //0456350000
  header.version = 257;
...
int main(int argc, char *argv[]) {
  ...
  while (argc) {
    ...
    // Parse options
      ...
      option_verbose = 1;
    } else if (!strcmp(argv[0], "-t")) { //mrdude test -do some stuff here
      print_myfile();
      exit(0);
  ...
  if (magic == 1414415231) { // PS4 PKG file 544e437f - 7f434e54
    fseek(file, get_ps4_pkg_offset(), SEEK_SET);
  } else if (magic == 1128612691) { // Disc param.sfo 43454353 - 53434543
    fseek(file, 0x800, SEEK_SET);
  } else if (magic == 1179865088) { // Param.sfo file 46535000 - 00505346
    rewind(file);
  } else {
